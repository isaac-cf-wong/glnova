...
def temp_config_with_accounts(temp_config_file: Path) -> Path:
    ...
    config = {
        "accounts": {
            "account1": {"name": "account1", "token": "token1", "base_url": "https://gitlab.com"},
            "account2": {"name": "account2", "token": "token2", "base_url": "https://gitlab.enterprise.com"},
        },
...
class TestListCommand:
    ...
    def test_list_accounts_shows_base_urls(self, temp_config_with_accounts: Path) -> None:
        ...
        assert re.search(r"account1.*https://gitlab\.com", result.stdout, re.DOTALL)
        assert re.search(r"account2.*https://gitlab\.enterprise\.com", result.stdout, re.DOTALL)

    def test_list_no_default_account(self, temp_config_file: Path) -> None:
        """Test listing when no default account is set."""
        config = {
            "accounts": {"account1": {"name": "account1", "token": "token1", "base_url": "https://gitlab.com"}},
